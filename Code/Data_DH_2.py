...
def data_read(s, r):
    ...
    vibr_all_normal = pd.DataFrame(vibr_all_fault1).reset_index(drop=True).values.reshape((-1, 1))
    ...
    X_dataset = pd.concat([pd.DataFrame(X_dataset)], axis=0).reset_index(drop=True).values.reshape([-1, 1])
    y_dataset = X_dataset[s:(num_samples-1)*s]
    X_dataset = X_dataset[0:(num_samples-2)*s]
    ...
    y_data = pd.concat([pd.DataFrame(y_dataset[0:num_samples * s])], axis=1).reset_index(drop=True).values.reshape((-1, s, 1))

    label = pd.concat([Label["L0"].loc[0:num_samples / 6 - 1], \
...
def data_embedding(x, y, seq_len):
    ...
    for i in range(0, len(x)):
        x_, y_ = x[i], y[i]
        x_ = pd.DataFrame(x_).values.reshape((8, int(seq_len / 8)))
        X_data.append(x_)
        y_ = pd.DataFrame(y_).values.reshape((8, int(seq_len / 8)))
        y_data.append(y_)
